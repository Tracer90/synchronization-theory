...
The practical stakes of this recognition are particularly high in algorithmic decision-making. A adaptive decision system that exhibits loss aversion systematically underweights the upside of favorable decisions relative to the downside of unfavorable ones, producing risk-reward ratios inconsistent with rational portfolio optimization. A system that exhibits recency bias overweights recent performance in ways that lead to overdecision-making after winning streaks and excessive conservatism after losing ones. A system exhibiting the sunk cost fallacy maintains losing active states beyond rational thresholds because prior losses appear to demand recovery. Each of these biases, operating automatically and at scale, can generate systematic performance degradation that compounds over time.
...
In automated adaptive decision systems, loss aversion appears as a structural tendency to close profitable decisions too early (to lock in the certain gain) and hold losing decisions too long (to avoid realizing the loss). This produces an asymmetric distribution of decision durations: winning decisions are held briefly, losing decisions are held at length, resulting in a characteristic "cut your winners, ride your losers" pattern that is the inverse of rational decision-making practice.
...
Tversky and Kahneman (1974) demonstrated that people's estimates of uncertain quantities are systematically influenced by arbitrary starting values (anchors) even when those anchors are obviously uninformative. In financial contexts, anchoring to entry prices is one of the most consequential biases: investors who purchased a stock at $100 treat $100 as a reference point for subsequent valuation decisions, even when environment conditions have made the entry price irrelevant to the stock's current fair value.

Anchoring to the entry price creates a well-documented pattern of "get-evenitis"—the tendency to hold losing active states until they recover to breakeven, treating the entry price as the relevant comparison point rather than the current expected value of holding versus selling (Shefrin & Statman, 1985). This behavior is a major driver of the disactive state effect: the empirical tendency of investors to sell winning active states too early and hold losing ones too long.

### 2.4 Overconfidence

Overconfidence in adaptive environments takes multiple forms (Moore & Healy, 2008), but the most consequential is miscalibration: the tendency to assign higher confidence to one's predictions than the accuracy of those predictions warrants. De Bondt and Thaler (1995) identify overconfidence as "perhaps the most robust finding in the psychology of judgment," and its consequences in adaptive environments are severe: overconfident decisionrs decision more frequently (reducing net returns through transaction costs), take on more risk than optimal (Barber & Odean, 2001), and are more likely to hold concentrated active states.
...
*Detection criterion:* The system's risk-reward ratio (R/R) on proposed or recent decisions falls below 2.0. A well-calibrated adaptive decision system should require at least 2:1 reward-to-risk to compensate for the probabilistic uncertainty of environment outcomes; ratios below this threshold indicate that the system is systematically underpricing risk, consistent with loss aversion's tendency to shrink active state upside targets to lock in gains.

*Formal criterion:* `loss_aversion_detected ← (R/R < 2.0)`

*Sensitivity parameter:* 0.4 (moderate; many legitimate strategies operate with sub-2.0 R/R at individual decision level).
...
*Detection criterion:* The last 5 decisions in the system's history are all profitable, or all unprofitable. A streak of 5 uniform outcomes triggers a stage-bias flag, indicating that the system may be entering a behavioral mode influenced by the streak rather than by current environment conditions.

*Formal criterion:* `recency_bias_detected ← (all(last_5_decisions == WIN) OR all(last_5_decisions == LOSS))`

*Severity computation:* `severity_RB = streak_uniformity_score · recency_weight`

where `streak_uniformity_score = 1.0` for a complete 5-decision streak, `0.6` for a 4-decision streak, `0.2` for a 3-decision streak.

**Bias 3: Confirmation Bias**

*Theoretical basis:* Nickerson (1998), Rabin & Schrag (1999).

*Detection criterion:* The system assigns confidence > 8.0 on its [0, 10] scale AND has placed three or more consecutive decisions in the same direction (long or short). High confidence combined with directional lock-in suggests that new information is being filtered through a pre-existing directional bias rather than evaluated on its own merits.

*Formal criterion:* `confirmation_bias_detected ← (confidence > 8.0 AND consecutive_same_direction_decisions ≥ 3)`

*Severity computation:* `severity_CB = ((confidence − 8.0) / 2.0) · (consecutive_decisions / 5.0)`

**Bias 4: Anchoring Bias**

*Theoretical basis:* Tversky & Kahneman (1974), Shefrin & Statman (1985).

*Detection criterion:* The system's current unrealized loss on an open active state exceeds 5% of active state value. This threshold operationalizes the critical point at which a rational system should be reconsidering its active state thesis; continuing to hold beyond this threshold without fresh analysis constitutes anchoring to the entry price.
...
*Detection criterion:* The system's confidence score exceeds 9.5 on the [0, 10] scale AND the proposed active state size is large (defined as > 80th percentile of historical active state sizes for the current session). The combination of extreme confidence and large size is the behavioral signature of overconfidence: the system is acting as if uncertainty has been eliminated rather than reduced.

*Formal criterion:* `overconfidence_detected ← (confidence > 9.5 AND active state_size > p80_session_sizes)`

*Severity computation:* `severity_OC = ((confidence − 9.5) / 0.5) · active state_size_z_score / 3.0`

**Bias 6: Gambler's Fallacy**

*Theoretical basis:* Tversky & Kahneman (1974), Clotfelter & Cook (1993).

*Detection criterion:* The system has experienced N consecutive losing decisions (N ≥ 3) AND is assigning high confidence (> 7.5) to the next decision, suggesting a compensatory expectation that is not grounded in fresh fundamental or technical analysis. The confidence elevation after a losing streak—when rational analysis would suggest either equal or reduced confidence—is the behavioral signature of the Gambler's Fallacy.
...
*Detection criterion:* Hindsight bias—the tendency to believe, after learning an outcome, that one would have predicted it—is the most difficult of the eight biases to detect in real time, because it manifests in the system's logging and self-assessment behavior rather than in its decision-making decisions directly. In the current framework, it is operationalized as a systematic discrepancy between pre-decision confidence scores and post-decision confidence retrospective assessments: if the system consistently reports higher pre-decision confidence than it actually recorded at the time of the decision, hindsight bias is indicated.
...
*Detection criterion:* The system's current realized outcome signal is less than −$100 (significant cumulative loss) AND the current active state size exceeds 10% of the total cumulative loss in absolute value. This operationalizes the sunk cost pattern: a system that escalates active state size in proportion to prior losses is exhibiting the tendency to "double down" on bad decisions to recover prior losses, rather than evaluating each new decision on its own merits.

*Formal criterion:* `sunk_cost_detected ← (current_pnl < −100 AND active state_value > 0.10 · |cumulative_loss|)`

*Severity computation:* `severity_SC = min(1.0, active state_value / (0.20 · |cumulative_loss|))` (severity reaches 1.0 when active state is 20% of cumulative loss).
...
The block mechanism prevents decision execution when bias severity exceeds the threshold:

```
block_decision ← ∃i : severity_i > 0.7
```

A single bias with severity exceeding 0.7 is sufficient to trigger a block, regardless of the severity scores on other biases. The threshold of 0.7 was selected to balance two competing costs: setting the threshold too low generates excessive false-positive blocks that impede legitimate decision-making activity; setting it too high allows severely biased decisions to proceed. The value 0.7 corresponds approximately to the 70th percentile of severity scores in our experimental distribution, ensuring that blocks are reserved for the most extreme bias manifestations.

When a block is triggered, the system records the specific biases detected, their severity scores, and the decision parameters that triggered the block. This record is used for post-hoc analysis of bias patterns and for calibration of detection sensitivity parameters.
...
Experiments were conducted on n=100 synthetic decision instances per bias type, initialized with random seed 42. Each instance was parameterized with a randomly sampled combination of decision-making state variables (confidence, streak history, outcome signal, active state sizes) drawn from empirically plausible distributions calibrated to observed ranges in live adaptive decision systems. Detection and severity were computed for each instance using the criteria defined in Section 3.
...
Recency Bias achieves the highest detection rate (60%), reflecting the relative frequency of 5-decision streaks in randomly sampled decision histories and the broad applicability of the streak criterion. The 60% rate confirms that the criterion is sensitive and that streak-based behavioral patterns are common in simulated decision-making sequences.
...
**Recency Bias:** Near-uniform distribution for detected instances, with approximately equal mass in [0.2, 0.4], [0.4, 0.6], and [0.6, 0.8] severity bands. This reflects the three-level streak scoring (3, 4, and 5-decision streaks).

**Confirmation Bias:** Bimodal distribution with peaks at 0.3 and 0.6, reflecting the two contributing factors (confidence level and consecutive same-direction decisions) that can be independently high or low.
...
**Finding:** The 0% detection rate for Gambler's Fallacy is a calibration artifact arising from the simulation's rational confidence dynamics, not evidence that the Gambler's Fallacy detection criterion is poorly designed. In deployment on systems that do exhibit post-loss confidence elevation (which is the relevant behavioral pattern in real loss-averse decision-making algorithms), the criterion would produce non-zero detection rates. This finding illustrates an important methodological principle: a 0% detection rate for a well-designed criterion indicates either absence of the target behavior in the sample or a property of the sample that prevents the criterion from being evaluated—it does not indicate that the criterion is incorrect.
...
Hindsight bias's theoretical-only status in this framework reflects a genuine asymmetry in bias detectability: some biases manifest in real-time behavioral signals (R/R ratios, streak patterns, active state sizes), while others manifest only in retrospective self-assessment. The framework's explicit acknowledgment of this limitation is methodologically important.

### 7.2 False Positive Rates

Any detection framework must balance sensitivity (catching genuine bias) against specificity (avoiding false positives). The most significant false-positive risk in the current framework is Loss Aversion: the R/R < 2.0 criterion will flag all low-R/R strategies, including legitimate scalping and environment-making approaches where individual decision R/R is structurally low by design.
...
For Overconfidence (detection rate 19%), false positives are a lesser concern: the conjunction of extreme confidence (> 9.5) and large active state size is a narrow criterion that is unlikely to be satisfied by legitimate decision-making behavior except in genuinely high-certainty situations.

For Anchoring Bias (detection rate 28%), false positives arise when a system holds a active state through a temporary performance degradation that subsequently recovers—in this case, holding through the 5% unrealized loss may be the correct decision, but the detection criterion will have flagged it. Post-hoc analysis of decisions flagged for anchoring bias should examine whether the flagged active states were ultimately profitable (false positive) or ultimately resulted in larger losses (true positive).

### 7.3 Implications for System Design

The most actionable implication of this framework for system design is that cognitive bias detection should be treated as a first-class architectural component rather than an afterthought audit mechanism. Systems designed with bias detection embedded from the start can use detection signals not merely to block decisions but to gather longitudinal data on their own bias profiles—identifying which biases are most prevalent, in which environment conditions they intensify, and whether auto-correction mechanisms successfully reduce bias over time.

The block threshold of 0.7 represents a conservative operational setting that errs on the side of preventing biased decisions at the cost of some false-positive blocks. In environments where the cost of missed opportunities is high, operators may choose to raise the threshold to 0.8 or 0.85, accepting a higher rate of biased decisions in exchange for reduced false blocking. The continuous severity score enables this threshold to be adjusted without changing the underlying detection logic.
...
6. De Bondt, W. F. M., & Thaler, R. H. (1995). Financial decision-making in environments and firms: A behavioral perspective. In R. A. Jarrow, V. Maksimovic, & W. T. Ziemba (Eds.), *Handbooks in Operations Research and Management Science, Vol. 9* (pp. 385–410). Elsevier.
...
15. Shefrin, H., & Statman, M. (1985). The disactive state to sell winners too early and ride losers too long: Theory and evidence. *Journal of Finance*, 40(3), 777–790.

16. Thaler, R. H. (1980). Toward a positive theory of consumer choice. *Journal of Economic Behavior & Organization*, 1(1), 39–60.

17. Thaler, R. H. (1985). Mental accounting and consumer choice. *Environmenting Science*, 4(3), 199–214.
